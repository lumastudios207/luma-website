...
const QUESTION_LABELS: Record<string, string> = {
  ...
  q7: 'Number of locations',
  q8: 'Google Business Profile status',
  q9: 'Average Google star rating',
  q10: 'New Google reviews in past 3 months',
  q11: 'Review process',
  q12: 'Website content activity',
  q13: 'Dedicated service + location landing pages',
  q14: 'Instagram / social activity and visual content',
  q15: 'NAP consistency across directories',
  q16: 'Top priorities for SEO',
  q17: 'Approximate monthly SEO budget',
};
...
const formatValue = (v: unknown): string => {
  if (Array.isArray(v)) return v.map((x) => escapeHtml(String(x))).map((x) => `• ${x}`).join('<br>');
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const { q1, q2, q4, recaptchaToken } = body;
    ...
    if (!q4 || typeof q4 !== 'string' || q4.trim().length < 3) {
      return new Response(JSON.stringify({ error: 'Invalid website' }), { status: 400 });
    }
    if (!recaptchaToken) {
    ...
    await resend.emails.send({
      ...
      subject: `Free SEO Audit request from ${String(q1).trim()} — ${String(body.q3 ?? '').trim()}`,
      html: `
