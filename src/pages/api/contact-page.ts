...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    await resend.emails.send({
      ...
      replyTo: email.trim(),
      subject: `Contact page inquiry from ${name.trim()} — ${company.trim()}`,
      html: `
        <h2>New Contact Page Inquiry</h2>
        <p><strong>Name:</strong> ${name.trim()}</p>
        <p><strong>Email:</strong> ${email.trim()}</p>
        <p><strong>Company / Business:</strong> ${company.trim()}</p>
        <p><strong>Message:</strong></p>
        <p style="background:#f5f5f5;padding:12px;border-radius:4px;">${message.trim().replace(/\n/g, '<br>')}</p>
        <hr>
